...
allowed-tools: bash write read subagent
...
- **Worktree agent**: a pi session running in its own git worktree/branch
...
- If delegating a skill (e.g., `/skill:auto`), instruct the agent to use it. Do
  not write detailed implementation steps yourself
...
workmux send agent-a "/skill:merge"
...
Tell the agent to merge its own branch via `/skill:merge`. This lets the agent
...
workmux send auth-module "/skill:merge"
workmux wait auth-module --timeout 120
workmux send api-tests "/skill:merge"
...
workmux send docs-update "/skill:merge"
```
...
5. **Merge one at a time** by sending `/skill:merge` to each agent sequentially.
   Wait for each merge to complete before starting the next to avoid conflicts.
